...
(function buildLanes() {
  ...
  const isl = (x, z, s, col) => {
    ...
    const m = new THREE.Mesh(new THREE.ConeGeometry(2.6 * s, 1.6 * s, 6), new THREE.MeshBasicMaterial({ color: col }));
    m.position.y = 0.8 * s; g.add(m);
    g.position.set(x, -0.15, z); return g;
  };
  scene.add(isl(-55, -150, 4, 0x5a8f6a)); scene.add(isl(45, -170, 5, 0x6b9e77)); scene.add(isl(70, -120, 3, 0x5a8f6a));
})();
